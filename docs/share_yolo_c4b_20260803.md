...
| **7** | **일반 장애물(볼라드·전봇대 등) 클래스 추가 여부** | 현재 클래스는 `person`·`stairs` 둘뿐. 후보인 [AIHub 인도보행 영상](https://aihub.or.kr/aihubdata/data/view.do?dataSetSn=189)을 **샘플 실측(8/3)**: 장애물 29종(이동체 13·고정체 16) 확보 가능하나 ① **계단 클래스 없음** ② **야간 없음** — 야간 장애물 성능은 `C2` 촬영분에 장애물 장면이 있어야만 판정 가능 | **회의에서 결정 (결정 전 착수 안 함).** 추가하는 경우의 경로는 준비돼 있다: AIHub 29종 원 스키마 + `stairs` 채택(자체 촬영 라벨 가이드로 재사용) · person ID 통일 · `C4c` 재학습은 기존 성능 회귀 없음을 게이트로 · 600GB 아닌 부분 다운로드 (→ [data.md 3-1](data.md)). **비용의 본체는 학습이 아니라 촬영 코스에 장애물 장면을 넣는 것**(안건 3 연동)이다 |
...
채택 가중치를 **ONNX 로 내보내 공유 가능한 상태**로 만들어 뒀다 (8/3).
