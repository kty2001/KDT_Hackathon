...
🆕 **`2 bollard` 가 새로 생긴다** — 볼라드(차량 진입 억제 말뚝)다. ⚠️ **④ 강조 렌더에
이 클래스의 색이 아직 정의돼 있지 않다** (`scripts/emphasize.py` 의 `PALETTE`).
그대로 두면 흰색으로 그려져 `person`(시안)·`stairs`(노랑)와 구분되지 않는다 —
**저시력 사용자에게는 색 구분이 기능 요건**이므로 앱 적용 전에 정해야 한다.
