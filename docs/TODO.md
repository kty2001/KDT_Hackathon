...
> 최초 작성 **2026-07-31** · 갱신 **2026-08-23** (`C4e` **S0·S0b·S1·S2·S3 전부 완료** — 남은 것은 `C5` 판정)
...
| **S4** | `C5` 자체 야간 평가셋에서 **최종 판정** — 🆕 후보가 3개가 됐다(`c4b_loli0` 현 배포 · `c4d_11n_640` 인수분 · **`c4e_s3_11n` 자체**) | 0 | `C2`→`C3`→`C5` |
...
🔴 **S0b(8/23)로 방향이 바뀌었다** — 인수분 R1 의 무릎 `0.10` 은 **주간 판단**이고,
보행 시점 실야간 음성에서는 같은 하향에 `stairs` 오탐이 **2~3.7배**, 발화 프레임이
**6/15 → 14/15** 가 된다. **단일 운영 conf 는 0.25 를 유지**하고, 일괄 하향은 **시간축
흡수가 붙은 뒤** 재평가한다. 즉 `W6`/`P1` 이 S1 의 **선행조건**이 됐다.

🆕 **1순위 카드는 클래스별 conf 다.** 두 클래스가 conf 를 정반대로 요구한다 —
`bollard` 는 가까운 쪽이 **0.321** 까지 내려가 0.25 를 겨우 넘고, `stairs` 는 0.25→0.10
에서 오탐이 2~3.7배다. **단일 임계로는 둘을 동시에 못 만족시킨다.** `bollard` 만 낮추는
후처리는 모델을 안 건드린다 (→ [detection.md 9-7](detection.md)).

- ~~운영 conf `0.25 → 0.10`~~ → **보류** (인수분 R1 의 무릎: bollard **+7.4~8.5%p 에 FP 2.6~3.2배** — 주간 기준)
- FP 증가는 **시간축으로 흡수**한다 — `W6`(프레임 스킵 + 트래킹 보간) · `P1`(박스 hold·IIR).
  ⚠️ **`temporal_eval.py` 를 재사용처로 착각하지 말 것** — 그 스크립트는 ② **화소** 지표
  (플리커·고스팅) 전용이고 **YOLO 를 호출하지 않는다.** 쓸 수 있는 것은 프레임 로더
  (`frame_paths`·`load_sequence`) 뿐이다. hold 자체는 `pipeline_demo.py:126-134` 에
  **이미 구현돼 있고**, 없는 것은 **재는 자**다 → E1 (아래)
- 게이트: **프레임당 오탐 개수**(④가 화면을 덮지 않을 것) · rec34 person recall 무회귀

**✅ 실험 설계 완료 (8/23) — 두 갈래 · 원문은 → [detection.md 9-9](detection.md)**

| | 무엇 | 학습 | 선행 | 단독 성립 | 설계 |
|---|---|---|---|---|---|
| **E2** | **클래스별 conf** — `bollard` ∈ {0.25,0.15,0.10} × `stairs`·`person` 0.25 고정 × 4런 × 밝기 3단. `eval_real_night.py` 에 `--class-conf` 추가(기존 `--conf` 경로 보존) | 0 | 없음 | ✅ | 9-9-1 |
| **E1** | **박스 시간축** — 신설 `scripts/track_eval.py`. `--interp none\|hold\|track` × `--detect-every 1,2,3`, **오라클(매 프레임 탐지) 대비** 5지표. 소재 rec38 + `test_real_data/` mp4 5개 | 0 | 없음 | ✅ | 9-9-2 |

**순서는 E2 → E1.** E2 는 `stairs` 를 안 건드려 단독 성립하고 비용이 훨씬 싸다.
*일괄* 하향(0.25→0.10)만이 E1 을 선행조건으로 갖는다.

✅ **E2 완료 (8/23)** — `bollard` 0.15 가 채택안. **대가는 거의 0**(`stairs` 무증가 ·
`bollard` 오탐 `noneg` 1박스뿐)이나 **이득은 `c4d_11s_640` 에서만** 난다(`gain0.20` 1→2).
`11n` 은 이미 0.25 에서 다 잡아 **살 여유가 없고**, `noneg` 는 **역효과**다.
→ **`11s` 채택 시 이 카드가 그 런의 유일한 결격을 절반 메운다** (→ [detection.md 9-9-1](detection.md)).

🔴 **E1 완료 (8/23) — `S1` 은 여기서 끝난다.** `W6` 트래킹 보간은 `k=2` 에서 hold 대비
IoU **+0.013~0.017** 뿐이고 miss·ghost·깜빡임이 **완전히 동일**해 **불필요**하다.
`P1` 박스 평활은 **전 구간 값이 없다**. 🔴🔴 그리고 **게이트 3 이 실패했다** — 음성 영상에서
오탐 **평균 지속이 13.6 → 16.8 → 24.7 프레임**으로 늘어난다. **시간축은 오탐을 흡수하기는
커녕 더 오래 붙든다** → ***일괄* conf 하향은 보류가 아니라 기각**이고 **단일 conf 0.25 확정**이다.
★ **진짜 문제는 탐지기의 프레임 단위 불안정성**(오라클 자신이 1,000프레임에 105~127회
깜빡임)이고, 이것은 보간이 아니라 **`S2`·`S3`(데이터·학습)** 에서 풀 문제다
(→ [detection.md 9-9-2](detection.md)).

★ **0단계는 실행이 0으로 끝났다** — 인수분 `error_analysis/<런>/conf_sweep.csv` 에 640 축
4런 스윕이 이미 있었다. `11s` 가 **주간에서도 오탐 축 1위**(0.25 에서 bollard FP 953 ·
하향 배율 2.6배로 최소)라 9-6 야간 음성과 방향이 같다. ⚠️ **`stairs` 행이 없다**
(`bollard_val_2146` 에 GT 0) — `stairs` conf 근거는 야간 음성 15프레임이 유일하다.
⚠️ **최상위 `conf_sweep.csv` 는 `c4d_26n_960` 단일 런**이다(내용 동일). 대표값으로 쓰지 말 것.

⚠️ **E1 트래커** — `model.track()` 은 **데스크톱 검증용**이다. 배포 ONNX 는 NMS 조차 그래프
밖이라 실제 이식물은 **앱 코드 레벨 IoU 트래커**여야 한다. 하네스에 둘을 나란히 둔다.
보조 의존성(`lapx` 등)이 필요하면 **`uv add` 로 명시 추가** — 자동설치는 3장 함정 9 재현이다.

#### S2. 데이터 — 음성 표본 보강 🔴

★ **정정 (8/23) — 이 단계 전체가 대기 중인 것이 아니다.** 아래 5개 중 **3번은 선행조건이
0이고 지금 실행할 수 있다.** `W5`·`C3` 는 1·2·4 번에만 걸린다.
**`D:\datasets\bammasil_aihub_subset` 에 배경 음성 표본 `negative_night` 3,451장 /
521세션이 이미 있다** — 라벨이 **전부 0바이트**임을 확인했다(빈 파일 아닌 것 **0건**).
막고 있는 것은 데이터가 아니라 **배선 한 칸**이다 (→ [detection.md 11장](detection.md)).
...
3. 🟢 **계단 없는 보도·점자블록·맨홀·자전거도로를 배경 이미지로 투입 — 선행조건 0.**
   `noneg` 런이 이미 보여 줬다 — **배경의 효과는 mAP 가 아니라 FP 에 나타난다**
   (볼라드 FP +11% · person +16%). 배경 비율 **10% 유지**.
   ★ **소재는 이미 로컬에 있다** — `bammasil_aihub_subset/*/negative_night/`
   **3,451장 · 521세션 · 박스 0개**. `stairs` GT 가 0 인 보도 장면이라 **StairNet 에
   음성 표본이 없다는 8-1 의 결핍을 정확히 메운다.**
   ⚠️ 확인 명령 — 0 이 나와야 정상:
   ```powershell
   (Get-ChildItem "D:\datasets\bammasil_aihub_subset\*_labels\negative_night\labels\*.txt" |
...
| 🆕 **앱팀 계약 변경 공지** — 3클래스 전환 시 ONNX 출력 2→3 · `RUN_METRICS` 등록 · ④ `bollard` 색 미정의 (→ 3장 함정 16) | 팀장 | 💻 |
