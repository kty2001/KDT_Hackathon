...
> 최초 작성 **2026-07-31** · 갱신 **2026-08-13**
...
**남은 것은 3주 남짓이다** (8/13 기준). ①②③④ 네 단계는 전부 후보가 서 있고 데스크톱
...
| # | 작업 | 선행 | 환경 | 담당 | 근거 |
|---|------|------|------|------|------|
| **C2** | **자체 야간 촬영** — 핸드헬드·동적. ⚠️ 코스에 셋이 반드시 들어가야 한다 — **볼라드 장면** · **멀리서 계단에 접근하는 구간** · 🆕 **계단이 없는 야간 노면**(점자블록·자전거도로·자갈). 셋째는 8/13 실측 때문에 추가됐다: 실야간 노면에서 `stairs` 오탐이 되살아나는데 StairNet 에 음성 표본이 없어 **이것 없이는 못 고친다** | 없음 (🗣️안건 7 은 나가기 전 처리) | 🎥 | 팀원3 | [data.md 2-2](data.md) · [detection.md 8-4](detection.md) |
...
⚠️ **8월 1주 계획이 통째로 밀렸다.** `C2` 가 아직 안 나갔고 `P3` 은 여전히 0건이다.
`C4c` 만 외부에서 `C4d` 로 대신 진행됐다 — 즉 **크리티컬 패스는 한 칸도 안 움직였다.**

| 시기 | 🔴 크리티컬 | 🟢 병렬 |
|------|------------|---------|
| ~~8월 1주~~ | ~~안건 7 → `C2` 출발~~ **미이행** | `C4c` → 외부 `C4d` 로 대체 ✅ |
| **8월 2주 (지금)** | 🗣️**안건 7** → **`C2` 촬영 출발** — 더 못 미룬다 | **`P3` 앱 껍데기 착수** · `W5` 노면 받기 |
| **8월 3주** | `C3` 라벨링 → `C5` 평가셋 → **`C7b` 재판정** | `P4` B arm · `P5` 하우징·시연 촬영 |
...
1. 🔴 **`C2` 는 이미 1주 늦었다** (8월 1주 목표 → 미이행). 뒤에 4단계가 직렬로 매달려
   있어, 여기서 한 주 더 밀리면 `C9` 통합이 9월로 넘어간다.
