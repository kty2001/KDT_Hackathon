...
> 최초 **2026-07-31** · 갱신 **2026-09-03** — **③ 배포 모델 최종 확정 반영**, §0·§6·§7 을
> 현재 상태로 재작성(8/26 시점의 "크리티컬 패스 정체" 서술은 정리, git 이력에 원문 보존) ·
> §8 완료 이력에 8/26 이후 항목 보강.
...
①②③④ 네 단계 전부 채택안이 서 있고, **③ 배포 모델도 9/3 최종 확정**됐다
(`c4f_distill_11n_640` INT8 → [STATUS.md 갱신 박스](STATUS.md)). `C2` 자체 야간 촬영은
`data/own_night`(41장, 재라벨링 완료)까지 진행됐고 `C5` 판정도 8/30·9/1 두 차례 돌았다.
`P3` 앱은 별도 저장소(팀원2)라 이 repo 범위 밖이다. 남은 것은 **`C11` 실기기 계측**뿐 —
`model_selection.md` 6장이 "남은 불확실성" 1순위로 재확인한다.

★ ③ 는 외부(`C4d` 6런, 8/7 도착 → [STATUS ★인수분](STATUS.md))와 자체(8/23 첫 3클래스 런
→ 8/31 지식 증류 → 9/3 최종 채택) 두 갈래로 진행됐다.

> ⚠️ **직관과 반대인 순서 2건** — ① **② 방식 결정이 ③ 학습보다 뒤에 온다**(② 판정의 주 지표가
> ③ mAP 라 채점기가 먼저 있어야 한다) ② **자체 야간 촬영이 ② 방식 결정의 선행이다**
> (보유 데이터에 야간·강광원이 없어 평가 수단 자체가 없다).

> 📜 8/26 이전 시점의 "크리티컬 패스 정체" 서술과 세부 일정 리스크 판단은 이 문서의
> git 이력에 남아 있다 — 결정이 끝난 지금은 8장 완료 이력이 그 자리를 대신한다.
...
크리티컬 패스는 사실상 끝났다 — `C2` 촬영(41장) · `C4e`/`C5` 판정 · 증류 · 양자화 ·
**③ 배포 모델 최종 확정(9/3)**까지 전부 완료다. 남은 것은 하나뿐이다.

1. 🔴 **`C11` 실기기 측정 — 유일하게 남은 게이트.** 지속 FPS·발열·② 속도 게이트·최종
   확정 모델의 실기기 mAP를 전부 이 계측이 판정한다. `C10`(모바일 런타임 이식) 선행.
2. **`P3` 앱** — 별도 저장소(팀원2)라 이 저장소 범위 밖. 진행 상황은 이 문서로 추적하지 않는다.
...
| 작업 | 담당 | 환경 |
|------|------|------|
| 🔴 **`C11` 실기기 측정** — `C10` 이식 후, 최종 채택 모델(`c4f_distill_11n_640` INT8)로 진행 | 팀원2 | 📱 |
| **`P3` 앱** — 카메라→표시 루프 통합 | 팀원2 | 📱(별도 저장소) |
...
✅ **배포 conf 계약 정정 완료** — `c4f_distill_11n_640` FP32·INT8 패키지 모두 metadata
`conf: 0.25`로 확인됨(9/2 재확인). ✅ **`C2` 야간 촬영·`C4e` E3 재확인**은 완료 → 8장.
🟡 `W5`·`P4`·`W4`·`P2`·`P5`는 이 문서 최신화(9/3) 시점에 진행 여부가 커밋 이력으로
확인되지 않아 **상태 미검증**으로 남긴다 — 담당자 확인 필요.
...
| 항목 | 완료 | 근거 |
|------|------|------|
| ★★★ **③ 배포 모델 최종 확정 — `c4f_distill_11n_640` INT8** — `c4e_s3_11n`을 대체. 계약 동일(앱 수정 불요) · 문서 전체(STATUS·TODO·share·model_selection) 정리 | 9/3 | [STATUS 갱신 박스](STATUS.md) · [share 7-0c](share_yolo_c4b_20260803.md) |
| ★★ **`model_selection.md` 확장 — 최종 후보 명세 + own_night 41장 5자 재검증 + `stairs` 스케일 갭 정량분석** | 9/2 | [model_selection.md 7-8장](model_selection.md) |
| ★ **DL 파트 발표용 시각화 노트북 신설** — 학습곡선·증류 비교·`rec34` PR curve·후보 4종 비교 큐레이션 | 9/1 | `notebooks/dl_presentation_summary.ipynb` |
| ★★ **6종 재비교 + 속도 축 — `own_night` 41장(재라벨링 후)** — 재라벨링으로 mAP50 전 모델 0.29~0.33→0.51~0.59 급등 · INT8이 이 PC CPU에서는 오히려 더 느림(가속 경로 없음) | 9/1 | [detection.md 9-12](detection.md) · `notebooks/c5_own_night_6way_compare.ipynb` |
| ★ **6종 성능·용량 비교 — `own_night_arc` 45장** — `c4e_s3_11n` mAP50 1등, 교사 recall/F1 1등이나 ONNX 3.6배 큼 · `stairs` recall 전 6종 0~0.056 | 9/1 | [detection.md 9-11](detection.md) · `notebooks/c5_own_night_arc_6way_compare.ipynb` |
| ★ **`own_night_KU` 병합** — 27→45장(신규 18장 라벨링), 촬영순 리네임. cv2 vs 뷰어 방향 불일치 함정 발견 | 9/1 | `scripts/merge_own_night.py` · [data.md 2-2-1](data.md) |
| ★★ **지식 증류 실행 완료 + NightOwls rec34 교차 확인 + stairs 외부 테스트셋** — RunPod A100 교사·학생 학습 → ONNX/INT8 → own_night·rec34·외부 14장 3자 비교. **own_night(교사>학생)과 rec34(학생>교사) 순위가 뒤집힘**(원인 미상) | 8/31 | [runpod_distillation_20260831](runpod_distillation_20260831.md) |
| ★ **`C4e` E3 — 오염 검증 재실행, 기각 최종 확정** — val-only 스플릿으로 재측정, 8/26 결론(ROI 크롭 기각) 재현 확인 | 8/31 | [detection.md 9-9-3](detection.md) |
| ★★ **`C5` 1차 실행 — `c4e_s3_11n` 우세 확인 + INT8 포함 비교 노트북** — 자체 27장 4종 비교, 전 축 우세 | 8/30 | [detection.md 9-10](detection.md) · `notebooks/c5_own_night_review.ipynb` |
| 🟡 **배포 conf 계약 정정 — 절반**(0.35→0.25). 🔴 FP32 패키지 `metadata.json`은 아직 0.35 | 8/26 | → 7장 |
...
- 앱팀 인수인계(3클래스 계약 · INT8 · **최종 채택 모델 교체**) → [share 7-0c](share_yolo_c4b_20260803.md)
