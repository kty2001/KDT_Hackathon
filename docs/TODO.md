...
> 최초 작성 **2026-07-31** · 갱신 **2026-08-22** (안건 7 종결 · `C4e` 성능 개선 트랙 신설)
...
| 단계 | 무엇 | GPU 학습 | 선행 |
|---|---|---|---|
| **S0** | 🔴 **`c4d` 6런을 held-out 게이트에 태워 채택 판정** | **0** | 없음 |
...
| 작업 | 담당 | 환경 |
|------|------|------|
| 🔧 **`data/` Junction 5개 재연결** — 1분. 이것이 없으면 `stairnet_to_bbox`·`night_eval`·`inspect_datasets`·`compare_lowlight` 가 "데이터 없음"으로 죽는다 (→ [data.md 5-2](data.md)) | 누구나 | 💻 |
| 🔴 **`C4e` S0 — `c4d` 6런 held-out 게이트 판정** (학습 0 · 가중치·코드 전부 로컬에 있음) | 팀장 | 🖥️ |
