...
| **S4** | `C5` 자체 야간 평가셋에서 **최종 판정** — 🆕 후보가 3개가 됐다(`c4b_loli0` 현 배포 · `c4d_11n_640` 인수분 · **`c4e_s3_11n` 자체**). 🔴 **잴 하네스가 아직 없다**(↓ S4 절) | 0 | `C2`→`C3`→`C5` + **하네스 신설** |
...
#### 🆕 S4. 최종 판정 — 🔴 **잴 자가 아직 없다** (2026-08-25 확인)

후보 3개(`c4b_loli0` 현 배포 · `c4d_11n_640` 인수분 · `c4e_s3_11n` 자체)를 `C5` 자체 야간
평가셋에서 가르는 것이 마지막 단계다. 그런데 **라벨 있는 자체 촬영분을 먹는 하네스가 이
저장소에 없다.** 기존 셋은 전부 다른 것을 잰다.

| 기존 하네스 | 무엇을 재나 | 왜 `C5` 를 못 재나 |
|---|---|---|
| `compare_detect.py` | rec34 mAP50·recall + rec38 오탐 | 경로·분할이 **NightOwls 전용**(`eval_nightowls.py` 경유) · **대시캠 도메인** |
| `eval_real_night.py` | `test_real_data/` 오탐 박스 수 · 양성 장면 conf | 🔴 **라벨이 없다** — recall·mAP·FP*율*을 원리적으로 못 센다 |
| `eval_stairs_night.py` | StairNet 야간 `stairs` | 단일 클래스 · 외부 데이터셋 |

→ **신설: `scripts/eval_own_night.py`** (가칭)

- **입력** `C3` 라벨(YOLO txt) + 자체 촬영 이미지·프레임 · `--runs` 로 가중치 여러 개
- **출력** 후보 × **클래스별** recall·precision·mAP50 + **음성 프레임 FP 박스 수** 한 표
- **게이트** 기존 3축을 그대로 이어받는다 (person recall · `stairs` 야간 오탐 ·
  실야간 음성 박스 수) — 자를 바꾸면 8/23 판정들과 대조가 끊긴다
- ⚠️ **`eval_real_night.py` 의 `--class-conf` 경로를 재사용할 것.** 클래스별 conf
  채택안(`bollard` 0.15)이 살아 있는 후보(`c4d` 계보)가 있어 **같은 자로 재야 한다**
  (→ [detection.md 9-9-1](detection.md))
- ⚠️ **볼라드는 박스별로 볼 것** — 장면 단위 `maxconf` 만 보면 먼 쪽에 가려 가까운 쪽의
  confidence 붕괴가 안 보인다 (→ [STATUS 2장 5](STATUS.md))
- **선행조건 0 — 지금 쓸 수 있다.** `C3` 라벨이 오기 전에는 `test_real_data/` 에 손 라벨
  몇 장을 물려 **스모크만** 돌린다. 하네스가 촬영보다 늦으면 `C5` 가 하네스를 기다린다
...
| 🆕 🔴 **`C5` 판정 하네스 신설** — `scripts/eval_own_night.py`(가칭). 라벨 있는 자체 야간분에서 후보 3개를 **한 표로** 비교한다. 기존 하네스 3종은 **전부 `C5` 를 못 잰다** · **선행조건 0** (→ 3장 `C4e` S4) | 팀장 | 💻 |
