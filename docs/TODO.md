...
> 🔄 **갱신 2026-08-04** — `C4`·`C4b`·`W1` 완료 · `stairs` 야간/주간 분리 실측 ·
> **클래스 3종 확정(8/3)** · **AIHub 노면 `stairs` 발견(8/4)** 반영
...
클래스는 **`0 person`·`1 stairs`·`2 bollard` 3종으로 확정**(8/3)됐고 배선도 끝났지만
**학습 데이터가 없다** — `bollard` 는 샘플에 172박스뿐이라 **AIHub 부분 다운로드가
선행**이다. ★ **8/4**: 같은 AIHub 의 **노면 마스킹 태스크에 `stairs` 가 있어**
원거리 계단을 **주간 한정으로** 잴 수 있게 됐다 (야간은 여전히 `C2` 뿐 →
...
| **W5** | AI Hub 인도보행 부분 다운로드 — ★ **라벨(1·2번)은 선행조건이 없다. 지금 받을 수 있다** (이용 신청만) | ~~W2 완료~~ → **원천(3·4번)만 해당.** 라벨은 가볍고, 3·4 의 선택이 라벨 실측에 달려 있어 **먼저 받는 것이 순서다.** 특히 **2번(노면 라벨)** 을 빠뜨리면 `stairs` 판단이 통째로 밀린다 | [data.md 3-1-3](data.md) |
...
| 7 | **일반 장애물 — 클래스를 더 늘릴 것인가** (안건이 좁아졌다). 클래스는 **`0 person`·`1 stairs`·`2 bollard` 3종 확정**(8/3, 배선 완료)이라 남은 것은 **① 나머지 26종 추가 여부**(권고: **미채택** — `pole` 648px vs `bollard` 75px 로 형태가 갈려 버려도 안전) **② `2 bollard` 의 범위**(2장의 "전봇대"=`pole` 은 별개. 넓히려면 `--pole-as-bollard`) **③ 노면 20종 갈래**(`damaged`·`grating`·`manhole`·`braille_guide_blocks` — 전부 세그 클래스라 bbox 파이프라인과 태스크 불일치. 지금 결정 불요). ~~계단 없음~~ → **8/4 정정: 노면 태스크에 `stairs` 있음.** 무엇을 늘리든 **비용의 본체는 촬영 코스**(안건 3 연동) | **다음 회의** — 결정 후 착수 | [data.md 3-1-1·3-1-2](data.md) |

> ~~**3·4번은 촬영/라벨링 착수 전에 결정해야 한다.**~~ → **8/3 처리 완료. `C2` 촬영을
> 막는 것이 없어졌다.** 나머지는 C7 전후로 묶어도 된다.
>
> ⚠️ **안건 7(일반 장애물)이 아직 3·4에 얹혀 있다.** 추가로 결정되면 촬영 코스에
> 장애물 구간을 넣어야 한다(3). **`C2` 를 나가기 전에 7을 같이 처리하지 않으면
> 재촬영 위험**이 남는다 (→ [data.md 3-1-1](data.md)).
> ⚠️ **결정과 무관하게 촬영 코스에 이미 확정된 요구가 둘 있다** — **볼라드 장면**
> (AIHub 가 주간 전용이라 야간 볼라드는 `C2` 로만 판정된다) 과 **멀리서 계단에
> 접근하는 구간**(원거리 계단의 야간 답은 `C2` 뿐 — 주간은 AIHub 노면 `stairs` 로
> 미리 볼 수 있다, → [detection.md 8-3](detection.md)).
...
| `C4b` ③ 재학습 — held-out recall 0.195→0.609 · `stairs` 야간 오탐 7.7%→0.2% | 8/2 | [detection.md 6장](detection.md) |
| `C7` ② arm × ③ mAP → **표시/탐지 경로 분리 확정** | 8/2 | [detection.md 7장](detection.md) |
| 🗣️ 회의 안건 **3·4 처리** — `C2` 촬영 선행 조건 해소 · `stairs` 라벨 정의 확정 | 8/3 | [labeling_stairs.md](labeling_stairs.md) |
| **클래스 3종 확정 + AIHub 배선** · ③ **ONNX 배포 패키지**(앱팀 인수인계) | 8/3 | [data.md 3-1-1](data.md) · [share](share_yolo_c4b_20260803.md) |
| ★ AIHub **노면 마스킹 `stairs`** 발견 — 8/3 "계단 없음" 정정 | **8/4** | [data.md 3-1-2](data.md) |
...
| 찾는 것 | 문서 |
|---------|------|
| ★ **현재 상태 한 장** (여기부터 읽는다) | [docs/STATUS.md](STATUS.md) |
| 기획·현재 진행 상황 | [README.md](../README.md) |
| ★ `stairs` 라벨을 어떻게 치는가 (`C3` 작업자용) | [docs/labeling_stairs.md](labeling_stairs.md) |
| 팀 공유 — ③ 훈련 결과 + 회의 안건 + 앱팀 인수인계 | [docs/share_yolo_c4b_20260803.md](share_yolo_c4b_20260803.md) |
