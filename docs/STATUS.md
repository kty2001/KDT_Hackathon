...
> 갱신 **2026-08-26** — `C4e` **E3 ROI 크롭 기각**(+ 배포 conf 0.35→0.25 절반 정정) ·
> `C10a` ③ ONNX 양자화(**8비트 채택 · 4비트 기각**) + **함정 20** ·
...
| ★ **양자화 실측** — 파일 표 · FP32 정합 · **붕괴 검사**(INT8 통과 / INT4 기각) | `outputs/quantization/<pkg>/<pkg>_README.md` | — |
...
| ★★ **`C10a` ③ ONNX 양자화 — 8비트 채택 · 4비트 기각** (8/26) | **INT8 10.61→3.15MB** · 계약 `[1,7,8400]` 그대로 · 야간 볼라드 **2/2 유지** · 음성 오탐 5→4박스. 🔴 **INT4 는 붕괴**(`qnn` 은 conf 0.01 에서도 0검출 · `generic` 은 볼라드 2/2→0/2 · Conv 는 per-block 스케일 불가라 per-channel 이 상한) · 🔴 속도는 이 PC 로 판정 불가(`C11`) | [TODO `C10a`](TODO.md) · 함정 20 · 패키지 README |
