...
| `roi_crop_eval.py` | ★ `C4e` S1(E3) — **ROI 크롭이 작은 볼라드를 살리는가**. 🔴 **3-arm 이 요점** — YOLO 는 stride 8 이라 *박스가 커지기만 해도* 오르므로, **정보 증가가 0 인 arm**(이미 640 인 이미지를 크롭해 되키우기)을 끼워 **스케일 효과와 해상도 효과를 가른다**. 구간의 자는 **arm A 기준 고정** · recall 분모는 **프레임 전체**(창 밖 GT 도 미탐). 결과는 **기각** | `--windows` · `--union`(2패스) · `--window-cy` · `--n` |
