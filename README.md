...
**요약 (2026-09-03)** — ①②③④ 전부 채택안이 확정됐고 **③ 배포 모델도 최종 확정**됐다.
남은 것은 실기기 계측(`C11`)과 앱 이식(별도 저장소)이다.

| 단계 | 상태 |
|------|------|
| ① 눈부심 억제 | `D1`(Drago) 하이라이트 압축으로 후보 확보 — ②에 통합할지 🗣️ 회의 안건 |
| ② 저조도 개선 | **고전 CV 확정**(8/1) · **표시 경로 전용**(8/2). 잠정 1위 `D1A1+bf+ts`. ⚠️ **속도 게이트 경계**(실기기 미확정) |
| ③ 위험요소 탐지 | ✅ **최종 확정**(9/3) — `0 person` `1 stairs` `2 bollard`, **`c4f_distill_11n_640` INT8**(지식 증류, 3.15MB). 실야간 held-out(`rec34`) recall 0.195 → 0.609 → 0.635 → **mAP50 0.717**(재비교 1등). ONNX 3클래스 패키지 앱팀 전달(입출력 계약 `[1,7,8400]`, 이번 모델 교체에도 계약 불변) |
| ④ 선택적 강조 | ✅ 렌더·색 구현 완료 — 실기기 가독성 검증 대기 |
| 앱 | 별도 저장소(팀원2) — 이 저장소는 ML/데이터 파트만 다룬다 |

자세한 현재 상태·막힌 것은 [docs/STATUS.md](docs/STATUS.md), 순서·병렬성은 [docs/TODO.md](docs/TODO.md).
